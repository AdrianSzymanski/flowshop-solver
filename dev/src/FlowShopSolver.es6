var FlowShopSolver = function() {
  ...
  // helper functions
  var getJobsOrder = function(data, algorithm = 'default') {
    var jobs_order = [],
    ...
    if(machines_number === 2) two_machines_data = JSON.parse( JSON.stringify(data) );
    else if(machines_number%2 === 0) {
    ...
    if(algorithm === 'johnson') {
      ...
      var conditions = function(a, b) {
        if(a.times[0] <= a.times[1] && b.times[0] <= b.times[1]) {
          ...
          if(a.times[1] < b.times[1]) return 1;
          else if(a.times[1] == b.times[1]) return 0;
          else return -1;
        }
      };

      two_machines_data.sort(conditions);
      jobs_order = two_machines_data.map(function(row) { return row.jobID; });
    }
    else {
  ...
  var getRandomJobsOrder = function(data) {
    /*
    ...
    in_table = data.map(function(row) { return row.jobID; }); // i.e: [1, 2, 3, 4, 5]
  ...
  var getSolutionTable = function(data, jobs_order) {
    /*
    ...
    for(var i = 0; i < sorted_data.length; i++) {
      for (var j = 0; j < jobs_order.length; j++) {
        ...
        if(new_job.jobDuration !== 0)
          sorted_data[i].jobs.push(new_job);
      };
    };
  ...
  var getPreviousMachineFinish = function(sorted_data, machine_number, job_number) {
    /*
    ...
    for(var i = machine_number - 1; i >= 0; i--) {
      for(var j = sorted_data[i].jobs.length - 1; j >= 0; j--) {
        ...
        if(sorted_data[i].jobs[j].jobID === jobID)
          return sorted_data[i].jobs[j].jobStart + sorted_data[i].jobs[j].jobDuration;
  ...
  var getPreviousJobFinish = function(sorted_data, machine_number, job_number) {
    /*
  ...
  var getCMax = function(sorted_data) {
  ...
  var getCX = function(sorted_data, jobID) {
    var machines_number = sorted_data.length,
        maxID = 1,
        c_x = 0,
        temp = 0;
    ...
    for(var i = 0; i < machines_number; i++) {
      for(var j = 0; j < sorted_data[i].jobs.length; j++) {
        ...
        if(sorted_data[i].jobs[j].jobID > maxID)
          maxID = sorted_data[i].jobs[j].jobID;
    ...
    for(var i = 0; i < machines_number; i++) {
      for(var j = 0; j < sorted_data[i].jobs.length; j++) {
        ...
        if(temp.jobID === jobID && (temp.jobStart + temp.jobDuration) > c_x)
          c_x = (temp.jobStart + temp.jobDuration);
  ...
  var getFMean = function(sorted_data) {
    var f_mean = 0,
  ...
  var getEarlierMachineFinish = function(sorted_data, machine_number, job_number) {
    /*
    ...
    for(var i = machine_number - 1; i >= 0; i--) {
      for(var j = sorted_data[i].jobs.length - 1; j >= 0; j--) {
        ...
        if(sorted_data[i].jobs[j].jobID === jobID) {
          ...
          if(previous_job_finish === 0) continue;
          else return previous_job_finish;
        }
  ...
  var delayJobsOnCurrentMachine = function(sorted_data, jobs_order, machine_number, job_number, delay) {
    /*
    ...
    for(var i = 0; i < sorted_data[machine_number].jobs.length; i++) {
      if(sorted_data[machine_number].jobs[i].jobID === jobID) 
        job_position = i;
    }
  ...
  var delayJobsOnEarlierMachines = function(sorted_data, jobs_order, machine_number, job_number, delay) {
    /*
    ...
    for(var i = machine_number - 1; i >= 0; i--) {
      ...
      for(var k = 0; k < sorted_data[i].jobs.length; k++) {
        if(sorted_data[i].jobs[k].jobID === jobID)
          job_position = k;
      }
      /*console.log('job_position: '+job_position);*/
      ...
      if(job_position === null) {
        ...
        for(var k = 0; k < jobs_order.length; k++) {
          if(jobs_order[k] === jobID) 
            job_order_position = k;
            // 2
        }
  ...
  var findJobsMinIdleTime = function(sorted_data, jobID) {
    /*
    ...
    for(var i = 0; i < sorted_data.length; i++) {
      for(var j = 0; j < sorted_data[i].jobs.length; j++) {
        ...
        if(sorted_data[i].jobs[j].jobID === jobID) {
          ...
          if(previous_job_finish === sorted_data[i].jobs[j].jobStart) return 0;
          else {
            // for the first found idle_time
            if(min_idle === null)
              min_idle = sorted_data[i].jobs[j].jobStart - previous_job_finish;
            // for other idle times pick the smaller one
            else if(min_idle > sorted_data[i].jobs[j].jobStart - previous_job_finish)
              min_idle = sorted_data[i].jobs[j].jobStart - previous_job_finish;
          }
  ...
  var translateJob = function(sorted_data, jobID, translation) {
    /*
    ...
    for(var i = 0; i < sorted_data.length; i++) {
      for(var j = 0; j < sorted_data[i].jobs.length; j++) {
        ...
        if(sorted_data[i].jobs[j].jobID === jobID)
          sorted_data[i].jobs[j].jobStart -= translation;
  ...
  // main functions
  var solveNormal = function(data, jobs_order) {
    ...
    for(var i = 1; i < sorted_data[0].jobs.length; i++)
      sorted_data[0].jobs[i].jobStart = sorted_data[0].jobs[i - 1].jobStart + sorted_data[0].jobs[i - 1].jobDuration;
    ...
    for(var i = 1; i < sorted_data.length; i++) {
      for(var j = 0; j < sorted_data[i].jobs.length; j++) {
        ...
        if(j === 0)
          sorted_data[i].jobs[j].jobStart = getPreviousMachineFinish(sorted_data, i, j);
        else {
  ...
  var solveNoWait = function(data, jobs_order) {
    var sorted_data = getSolutionTable(data, jobs_order),
        machines_number = sorted_data.length,
    ...
    for(var i = 0; i < jobs_order.length; i++) {
      ...
      if(min_idle > 0)
        translateJob(sorted_data, jobs_order[i], min_idle);
    };

    return sorted_data;
  };

  var solveNoIdle = function(data, jobs_order) {
    var sorted_data = getSolutionTable(data, jobs_order),
        machines_number = sorted_data.length,
        earlier_machine_finish = 0,
        this_job_start = 0,
